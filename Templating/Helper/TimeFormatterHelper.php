...
class TimeFormatterHelper extends Helper
{
    ...
    private function formatNormal(\DateInterval $diff)
    {
        ...
        if ($diff->y > 0) {
            ...
            return $diff->invert ? $this->translator->trans("next half hour") : $this->translator->trans("half hour ago");
        } else if ($diff->i > 0) {
            $str = "minute";
            $val = $diff->i;
        } else if ($diff->s > 0) {
        ...
        if ($diff->invert) {
            return $this->translator->transChoice(
                ...
                ($second && isset($second['after']) ? $second['after'] : "next " . $id . " " . $str . "s"), $val, array(
                $id => $val
            ));
        } else {
            return $this->translator->transChoice(
    ...
    private function formatSimple(\DateInterval $diff)
    {
        ...
        return $this->translator->trans($diff->invert ? "next " . $str : $pre . $str . " ago");
    }
